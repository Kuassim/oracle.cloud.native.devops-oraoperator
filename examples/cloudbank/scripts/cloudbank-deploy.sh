...
# Build
state_set '.state.apps.cloudbank_frontend |= $VAL' DEPLOYING
(cd "$APP_FE_BUILD_SCRIPT_LOCATION" || exit ; sh "$APP_FE_BUILD_SCRIPT_FILENAME")
state_set '.state.apps.cloudbank_frontend |= $VAL' DEPLOYED
...
# Build
state_set '.state.apps.cloudbank_backend |= $VAL' DEPLOYING
(cd "$APP_BE_BUILD_SCRIPT_LOCATION" || exit ; sh "$APP_BE_BUILD_SCRIPT_FILENAME")
state_set '.state.apps.cloudbank_backend |= $VAL' DEPLOYED